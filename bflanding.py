...
def play():
    if not hasattr(root, 'logged_in') or not root.logged_in:
        ...
        login_window = LoginWindow(root, redirect_to_dashboard=True)
    else:
...
def help():
    # Create a top-level window for the Help page
    help_window = tk.Toplevel(root)
    help_window.title("Help")
    
    # Load and resize background image
    background_image = Image.open("bfgarage.jpg")
    background_image = background_image.resize((root.winfo_screenwidth(), root.winfo_screenheight()))
    background_photo = ImageTk.PhotoImage(background_image)
    
    # Display background image
    background_label = tk.Label(help_window, image=background_photo)
    background_label.image = background_photo
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
    
    # Font styles
    title_font = ("MS Gothic", 50)
    text_font = ("MS Gothic", 18)
    
    # Title
    title_label = tk.Label(help_window, text="Help Page", font=title_font, fg="black")
    title_label.pack(pady=(50, 20))
    
    # Help content (using Text widget for better control)
    help_text = """
    How to Play:
    ...
    help_text_widget = tk.Text(help_window, wrap=tk.WORD, font=text_font, bg="white", padx=18, pady=10)
    help_text_widget.insert(tk.END, help_text)
    help_text_widget.config(state=tk.DISABLED)
    help_text_widget.pack(fill=tk.BOTH)
    
    # Return to homepage button
    return_button = tk.Button(help_window, text="Return", command=help_window.destroy, height=3, width=15, font=("MS Gothic", 14))  # Increased button size
